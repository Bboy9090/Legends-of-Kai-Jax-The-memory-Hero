...
bool StoryModeManager::LoadConfiguration(
    ...
    try {
        ...
        for (const auto& district_json : story_json["districts"]) {
            ...
            district->traversal_features.verticality_enabled = traversal["verticality_enabled"].get<bool>();
            district->traversal_features.rooftop_access = traversal["rooftop_access"].get<bool>();
            district->traversal_features.interior_transitions = traversal["interior_transitions"].get<bool>();
            ...
            for (const auto& zone_json : district_json["zones"]) {
                ...
                zone->enemy_spawn_enabled = zone_json["enemy_spawn_enabled"].get<bool>();
            ...
            // Check if unlocked
            if (district_json["unlock_requirement"]["type"].get<std::string>() == "story_progress" &&
                district_json["unlock_requirement"]["value"].get<std::string>() == "start") {
