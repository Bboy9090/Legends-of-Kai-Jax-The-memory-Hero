...
export default function Player() {
  ...
  useFrame((state, delta) => {
    ...
    // Simple running animation - slight rotation to simulate motion
    if (modelRef.current && !player.isSliding) {
      const runBob = Math.sin(animationTimeRef.current * 10) * 0.05;
      modelRef.current.rotation.z = runBob;
    } else if (modelRef.current && player.isSliding) {
  ...
  return (
    <animated.group position={position as any}>
      <animated.group scale={scale as any} rotation={rotation as any}>
        ...
        <Points ref={particleRef} positions={particles} stride={3}>
          <PointMaterial
            transparent
        ...
        {/* Invisible reference mesh for legacy code compatibility */}
